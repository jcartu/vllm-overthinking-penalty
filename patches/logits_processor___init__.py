...
def build_logitsprocs(
    ...
    if vllm_config.speculative_config:
        ...
        # Overthinking penalty: a stateless fixed-penalty processor that is
        # safe with speculative decoding (no per-request state, pure logits
        # tensor modification). Enabled when OVERTHINKING_PENALTY_LAMBDA > 0.
        # See arxiv 2606.00206 — reduces CoT length 12-23% with no accuracy
        # loss on quantized reasoning models.
        try:
            import os
            import sys
            ...
            from overthinking_penalty import OverthinkingPenaltyProcessor
            processors.append(
                OverthinkingPenaltyProcessor(vllm_config, device, is_pin_memory)
            )
            logger.warning(
                "OverthinkingPenaltyProcessor loaded (lambda=%s, plugin_dir=%s)",
                os.environ.get("OVERTHINKING_PENALTY_LAMBDA", "5.0"),
                _plugin_dir,
            )
        except ImportError as e:
            logger.warning(
                "OverthinkingPenaltyProcessor import failed: %s — "
                "skipping (set OVERTHINKING_PLUGIN_DIR and install "
                "the module to enable).", e,
            )
